...
5. **Rule 105 FAILS** if a command-set ISE name equals a profile ISE name (YAML `name:` and Terraform POSTs), or if any ISE name is outside this locked unique list (underscore only). Command sets: `T1` `T2` `T3` `T4` `vendor` `contractor` `auditor_internal` `auditor_external` `test`. Profiles: `T1_shell` `T2_shell` `T3_shell` `T4_shell` `vendor_shell` `contractor_shell` `auditor_internal_shell` `auditor_external_shell`. CSV keys stay `T1`. Identity groups, NDGs, and authz rule names are unchanged.
